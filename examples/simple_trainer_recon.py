...
class Runner:
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        render_colors, render_alphas, expected_depths, median_depths, render_normals, info = rasterization(
            means=means,
            quats=quats,
            scales=scales,
            opacities=opacities,
        ...
        return render_colors, render_alphas, expected_depths, median_depths, render_normals, info
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            rade = cfg.normal_consistency_loss and (step>=15000)
            # forward
            renders, alphas, expected_depths, median_depths, normals, info = self.rasterize_splats(
            ...
            if rade:
                ...
                points_e = points_e.reshape_as(normals)
                points_m = points_m.reshape_as(normals)
                ...
                normal_error_map_e = (1 - (normals * normal_map_e).sum(dim=-1))
                normal_error_map_m = (1 - (normals * normal_map_m).sum(dim=-1))
                loss += cfg.normal_consistency_lambda * (0.4 * normal_error_map_e.mean() + 0.6 * normal_error_map_m.mean())
    ...
    def render_traj(self, step: int):
        ...
        K = torch.from_numpy(list(self.parser.Ks_dict.values())[0][None]).float().to(device)
        width, height = list(self.parser.imsize_dict.values())[0]

        canvas_all = []
        for i in tqdm.trange(len(camtoworlds), desc="Rendering trajectory"):
            renders, _, expected_depths, median_depths, normals, _ = self.rasterize_splats(
            ...
            # expected_depths = expected_depths[0]
            # expected_depths = (expected_depths - expected_depths.min()) / (expected_depths.max() - expected_depths.min())

            # median_depths = median_depths[0]
            # median_depths = (median_depths - median_depths.min()) / (median_depths.max() - median_depths.min())
            normal_map_m = normal_map_m[0]
            normal_map_m = (-normal_map_m + 1) / 2

            normal_map_e = normal_map_e[0]
            normal_map_e = (-normal_map_e + 1) / 2
            ...
            # write images
            # canvas = torch.cat(
            #     [colors, depths.repeat(1, 1, 3), normals], dim=0 if width > height else 1
            # )
            canvas = torch.cat(
                [
                    torch.cat([colors, normals], dim=1),
                    torch.cat([normal_map_m, normal_map_e], dim=1)
                 ], dim=0)
        ...
        writer = imageio.get_writer(f"{video_dir}/traj_{step}.mp4", fps=1)
    ...
    def recon(self, step: int):
        ...
        o3d.io.write_triangle_mesh(f"{mesh_dir}/recon_{step}.ply",mesh.to_legacy())
        print("done!")
